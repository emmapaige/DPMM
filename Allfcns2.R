                         ...
                         sitect = 5000, tests = 4, postD = 4 )
{
  ...
  save( rawdata, PP, newPP, tests, sitect, postD, 
        file = paste(current_working_directory,"/Data/TestData_",copy,".Rdata",sep=""))
...
LP_Emma <-
    ...
    #a is the even weight in the Dirichlet prior, whose concentration=5a.
  {
    ...
    lp = sum(Vec) + inf_sum_approx(u, K, Num, L) + u * lgamma(a*J) - J * u * lgamma(a)
...
################################
split<-function(data,G,s,a,Num,L)
{
  Y=data[,G]
  N=length(G)
  if (N==1) {MC=list(left=G,right=NULL)}
    ...
    po=LP_Emma(Y,Co,2,a,Num,L) #first occurence
    t=0
    absZ=10
...
#############################
LR<-function(Y,C1,C2,i,a,Num,L,K)
{
  U = length(unique(C1))                                
  W = length(unique(C2))
  A=which(C1==C1[i])
  ...
  LR=sum(lr)+lbeta(sum(M[B])-mi+a*J,mi)-lbeta(sum(M[A])-mi+a*J,mi) + 
    inf_sum_approx(W, K, Num, L) - inf_sum_approx(U, K, Num, L) 
...
blockcomb<-function(Z,Gp,s,a,bk,Num,L) #bk is the burnin number in terms of multiples of K.
{
  K=length(Gp)
  if (K==1) {print("one group")
    ...
    while (absZ>=2) #Check stationary using Geweke diagnostic.
    {
      ...
      d=exp(LR(Z,BC0,BC,i,a,Num,L,K=length(Gp)))
      u=runif(1)
      if (t<burn){
        if (u<d) {BC0=BC}
        else {BC0=BC0}	
...
#Gibbs
##############
LR.Gibbs<-function(Y,C,i,ki,a,Num,L,K) #i is the sequence index, ki is the new group label for position i. 
{
  #print(paste0("i=",i))
  #print(paste0("ki=",ki))
  #s=proc.time()
  ...
  + W*(lgamma(a*J)) - J*W*lgamma(a) - U*(lgamma(a*J)) + U*J*lgamma(a)
  #e= proc.time()-s
  #print(paste0("Time to complete one run of LR.Gibbs is ",e[1]+e[5]))
  return(LR) 
}


# GibbsGeweke<-function(Data,initialC,s,a,bk,Num,L)
# {
#   n=length(initialC)
...
#   {
#     i=ceiling(n*runif(1))
#     lrs=sapply(Kseq, LR.Gibbs, Y=Data, C=C0, i=i, a,Num,L,K)
#     rs=exp(lrs-max(lrs))
#     pp=rs/sum(rs)
#     cumPi=cumsum(pp)
#     u=runif(1)
...
#     {
#       tb=t-burn
#       LPs[tb%%s+1]=LP_Emma(Data,C0,K,a,Num,L)
#       Cs[tb%%s+1,]=C0
#       if(tb%%s+1==s)  
#       {
#         MClp=mcmc(LPs)
...
GibbsGeweke<-function(Data,initialC,s,a,bk,Num,L)
{
  ...
  C0=initialC  
  LPs=rep(NA,s)
  LP_new=rep(NA,700000) #new addition
  zs=rep(NA,500000)
  Cs=matrix(NA,nrow=s,ncol=n)
  absZ=10
  burn=K*bk
  ...
  while(absZ>=2)
  {
    ...
    {
      tb=t-burn
      LP_new[tb]=LP_Emma(Data,C0,K,a,Num,L) #new addition
      LPs[tb%%s+1]=LP_Emma(Data,C0,K,a,Num,L)
      Cs[tb%%s+1,]=C0
      if(tb%%s+1==s)  
      {
        ...
        if (is.na(z)){
          ...
          absZ=abs(z)
          zs[zcount+1]=geweke$z
        }   
        C=convC(Cs,LPs)
        MC=list(RunningC=Cs, LP=LPs, t=t, zscore=z, C=C,Gew=zs,LP_new)
        zcount=zcount+1
      }
    }
    t=t+1
  ...
  C=convC(Cs,LPs)
  MC=list(RunningC=Cs, LP=LPs, t=t, zscore=z, C=C,Gew=zs,LP_new=LP_new)
  return(MC)
}

# GibbsRandomScan<-function(Data,initialC,Scan,a,bk,Num,L)
# {
#   n=length(initialC)
#   K=length(unique(initialC)) # The max we are willing to consider is the number from block step?
#   Kseq=seq(1,K,1)
#   t=0
#   C0=initialC  
#   LPs=rep(NA,Scan)
...
#     {
#       tb=t-burn
#       LPs[tb+1]=LP_Emma(Data,C0,K,a,Num,L)
#       Cs[tb+1,]=C0
#     }
#     t=t+1
#   }
...
# GibbsFixScan.cont<-function(TempFile, Data, Scan, a, CORE = 2,Num,L)
# {
#   load(TempFile)
#   N=length(C0)
#   Kseq=seq(1,K,1)
...
GibbsFixScan<-function(Data,initialC,Scan,a, Record = F, CORE = 2, TempFile,Num,L)
{
  N=length(initialC)
  K=length(unique(initialC))
  Kseq=seq(1,K,1)
  ...
  while(Scan0 < Scan){
    for( ii in 1:N){
      lrs=simplify2array(mclapply(Kseq, LR.Gibbs, Y=Data, C=C0, i=ii, a = a, Num,L,K,mc.cores = CORE))
      rs=exp(lrs-max(lrs))
      pp=rs/sum(rs)
      cumPi=cumsum(pp)
      u=runif(1)
    ...
    Scan0 = Scan0 + 1
    if( Record == T ){
      LPs[Scan0] = LP_Emma(Data,C0,K,a,Num,L)
      Cs[Scan0, ] = C0  
    }
  }
  MC=list(ScanC=Cs, ScanLP=LPs, Scan=Scan, C = C0)
...
GibbsFixScan_Emma <- function(Data,initialC,Scan,a, Record = F, CORE = 2, TempFile,Num,L)
{
  print(paste0("number of cores is ",CORE))
  N = length(initialC)
  ...
  while(Scan0 < Scan){
    ...
    for( ii in 1:N){
      ...
      #start = proc.time()
      lrs=simplify2array(mclapply(Kseq, LR.Gibbs, Y=Data, C=C0, i=ii, a = a, Num=Num, L=L, K=K, mc.cores = CORE))
      #end = proc.time() - start
...
#######################
Gibbsresult3<-function(C0s,name,rawdata,GibbsMC=T)
{
  if( GibbsMC == T ){
    cputs = maxt = NULL
...
plotNiDi <- function( InfFile, ResultFile, SaveFile, SavePlot = F, PlotName = NULL,
  ...
  if( SavePlot == F ){
    ...
    abline(h=d, lty = 2)
    if(d0.value == T){text(n*.11, d+.5, substitute('d'[0] == dd, list(dd = d)), cex =.8 )
    }else{ text(0, d+.5, expression(bold('d'[0] )), cex =.8 )}
    if(Legend == T){
      legend("topright", inset=c(-0.3,0), legend = c("Ht(Di) ", " Ht(Ni)", "Signal"), pch = c(1,3,1),
             pt.cex = c( .5, .5, 1.5), cex = 1.2,
             col = c("darkorchid1", "blue", "red"), bty = "n", horiz = F, xpd = TRUE)
    } 
  }else{
    pdf( PlotName )
    plot(minHts.Dend, cex= .5, col = "green", ylim = Ylim,
...
#                       Main = "Summary Stats Plot", d0.value = T, Legend = T,
#                       Control = T ){
#   old_par <- par(mar = c(5.1, 4.1, 4.1, 6.1),
#                  cex.axis = 1.5,   # Axis annotation (tick labels) size
#                  cex.lab = 1.3,   # Axis label size (e.g., "Threshold d")
#                  cex.main = 1.7)  # Main title size
# 
#   load(InfFile); load(ResultFile); load(SaveFile)
#   datasets = length( Pairs ); n = length( mHts.pool )/datasets; D.length = length( PostD )
#   if( Control == T ){
...
#   Ylim = c(0,max(mHts.pos)+2)
#   if( SavePlot == F ){
#     plot(minHts.Dend, cex= .5, col = "darkorchid1", ylim = Ylim,
#          font.lab = 2, main = Main, xlab = "Genome site index", ylab = "Ht" )
#     points(signals, minHts.Dend[signals], cex= 1.5, col = "red")
#     points(maxHts.NoD, pch = 3, cex= .5, col = "blue")
#     abline(h=d, lty = 2)
#     if(d0.value == T){text(n*.11, d+.5, substitute('d'[0] == dd, list(dd = d)), cex =1 )
#     }else{ text(0, d+.5, expression(bold('d'[0] )), cex =1 )}
#     if(Legend == T){
#       legend(n*.1, max(Ylim) - .1, legend = c("Ht(Di) ", " Ht(Ni)", "Signal"), pch = c(1,3,1),
#              pt.cex = c( .5, .5, 1.5), cex = 1,
#              col = c("darkorchid1", "blue", "red"), bty = "n", horiz = T)
#     }
#   }else{
#     pdf( PlotName )
#     plot(minHts.Dend, cex= .5, col = "darkorchid1", ylim = Ylim,
...
#     abline(h=d, lty = 2)
#     if(d0.value == T){text(n*.11, d+.5, substitute('d'[0] == dd, list(dd = d)), cex =1 )
#     }else{ text(0, d+.5, substitute('d'[0] ), cex =1 )}
#     if(Legend == T){
#       legend(n*.1, max(Ylim) - .1, legend = c("Ht(Di) ", " Ht(Ni)", "Signal"), pch = c(1,3,1),
#              pt.cex = c( .5, .5, 1.5), cex = 1,
#              col = c("darkorchid1", "blue", "red"), bty = "n", horiz = T)
...
# 
# 
# PlotThreshold <- function(InfFile, ResultFile, SaveFile){
#   load(SaveFile)
#   load(InfFile)
#   load(ResultFile)
#   d1 <- d
#   max(mHts.pos)
#   
#   # Adjust the plot margins, font sizes, and line widths
#   old_par <- par(mar = c(5.1, 4.1, 4.1, 6.1),
#                  cex.axis = 1.5,   # Axis annotation (tick labels) size
#                  cex.lab = 1.3,   # Axis label size (e.g., "Threshold d")
#                  cex.main = 1.7)  # Main title size
#   
#   # Setting up the initial plot with thicker line
#   plot(Cuts, noise.test[1,], type = "l", lty = 1, col = "red", xlim = c(max(mHts.pos),0), ylim = c(0,100),
#        font.lab = 2, main = bquote(lambda == .(L)), xlab = "Threshold d", ylab = "# of sites", lwd = 2)
#   
#   # Adding lines for Potential and Noise with increased line width
#   lines(Cuts, noise.test[2,], lty = 2, col = "darkorchid1", lwd = 2)
#   lines(Cuts, noise.test[3,], lty = 6, col = "blue", lwd = 2)
#   
#   abline(v=d1, lty = 2, lwd = 2)  # Thicker line
#   
#   legend_labels <- c("Signal", "Potential", "Noise")
#   legend_line_styles <- c(1, 2, 6)
#   
#   # Position the legend using precise x and y coordinates
#   legend(x = max(mHts.pos) * .5, y = 30, legend = legend_labels, col = c("red", "darkorchid1", "blue"),
#          lty = legend_line_styles, lwd = 2, cex = 1.2, bty = 'n', seg.len = 1, x.intersp = 0.5)  
#   
#   par(old_par)  # Restore the previous plot settings
# }
...
#   if( SavePlot == F ){
#     plot(minHts.Dend, cex= .5, col = "darkorchid1", ylim = Ylim,
#          font.lab = 2, main = Main, xlab = "Genome site index", ylab = "Ht" , cex.main = 1.2, cex.lab = 1.2)
#     points(signals, minHts.Dend[signals], cex= 1.5, col = "red")
#     points(maxHts.NoD, pch = 3, cex= .5, col = "blue")
#     abline(h=d, lty = 2)
#     if(d0.value == T){text(n*.11, d+.5, substitute('d'[0] == dd, list(dd = d)), cex =1.2 )
#     }else{ text(0, d+.5, expression(bold('d'[0] )), cex =1.2 )}
#     if(Legend == T){
#       legend("topright", inset=c(-0.3,0), legend = c("Ht(Di) ", " Ht(Ni)", "Signal"), pch = c(1,3,1), 
#              pt.cex = c( .5, .5, 1.5), cex = 1.2, 
#              col = c("darkorchid1", "blue", "red"), bty = "n", horiz = F, xpd = TRUE)  
...
#     pdf( PlotName )
#     plot(minHts.Dend, cex= .5, col = "darkorchid1", ylim = Ylim,
#          font.lab = 2, main = Main, xlab = "Genome site index", ylab = "Summary statistics Ht" , cex.main = 1.2, cex.lab = 1.2)
#     points(signals, minHts.Dend[signals], cex= 1.5, col = "red")
#     points(maxHts.NoD, pch = 3, cex= .5, col = "blue")
#     abline(h=d, lty = 2)
#     if(d0.value == T){text(n*.11, d+.5, substitute('d'[0] == dd, list(dd = d)), cex =1.2 )
#     }else{ text(0, d+.5, substitute('d'[0] ), cex = 1.2 )}
#     if(Legend == T){
#       legend("topright", inset=c(-0.3,0), legend = c("Ht(Di) ", " Ht(Ni)", "Signal"), pch = c(1,3,1), 
#              pt.cex = c( .5, .5, 1.5), cex = 1.2, 
#              col = c("darkorchid1", "blue", "red"), bty = "n", horiz = F, xpd = TRUE)  
...
PlotThreshold <- function(InfFile, ResultFile, SaveFile) {
  ...
  # Increase right margin for the legend space
  #par(family = "sans", mar = c(5, 4, 4, 8))
  par(family = "sans")
  ...
  plot(
    ...
    lty = 1,
    col = "red",
    xlim = c(max(mHts.pos), 9),
    ylim = c(0, 20),
    font.lab = 2,
    main = "H1N1 Seg 8E1 (no Gibbs)",
    xlab = "Threshold d",
  ...
  # Adding lines for Potential and Noise
  lines(Cuts, noise.test[2, ], lty = 2, col = "green", lwd = 2)
  lines(Cuts, noise.test[3, ], lty = 6, col = "blue", lwd = 2)
  
  # Add the vertical line for the specific threshold value
  abline(v = d1, lty = 2, lwd = 2)
  
  # Create legend labels
  legend_labels <- c("Signal", "Potential", "Noise", expression(alpha == .05))
  legend_colors <- c("red", "green", "blue", "black")
  legend_line_styles <- c(1, 2, 6, 2)  # Assuming alpha = .5 uses lty = 2
  
  # Position the legend with improved spacing
  legend(
    "topleft",
    #inset=c(-0.2,0),  # Adjust inset to position the legend inside the plot area
    legend = legend_labels,
    col = legend_colors,
    lty = legend_line_styles,
    lwd = 2,
    cex = 1.3,  # Adjust for text size
    bty = 'n',
    #seg.len = 1.5,
    #x.intersp = 0.8,
    #y.intersp = 1.5  # Increase vertical interspacing
  )
  
  # No need to add separate lines for alpha; included in the legend
}
...
############################
#Below are the direct Gibbs functions for the Gibbs_wrapper.R

createDir <- function(dirName) {
  dir.create(paste(name, dirName, sep=""))
}

submitJobs <- function(numC0,current_iteration) {
  for (C0 in 1:numC0) {
    subFix=paste("sbatch --constraint=rhel8 -t 7- -N 1 -n ", CORE, " -J FixGibbs.", Jname, "[",C0,"] -o  ", name, "/SLURMouts/FixGibbs%a.out  --wrap=\"R CMD BATCH --vanilla --args --name=", name,
                 " --copy=",copy, " --core=", CORE, " --Ci=",C0, " --current_iteration=",current_iteration," FixGibbsMC_Emma_New_2.R ", name,"/Outputs/FixGibbs",C0,".out\"", sep="")
    system(subFix)
    Sys.sleep(1)
  }
}
...
loadMCMCObjects <- function(numC0) {
  mcmc_objects = vector("list", numC0)
  for (i in 1:numC0) {
    file_name = paste(name, "/FixGibbsMC", i, ".Rdata", sep="")
    load(file_name)
    assign(paste0("ScanLP_", i), mcmc(GibbsMC$ScanLP))
    mcmc_objects[[i]] = get(paste0("ScanLP_", i))
    rm(GibbsMC)
  }
  return(mcmc_objects)
}
...
gibbs_sampler <- function(y, alpha, max_clusters, s, burn, name, initialC) {
  ...
  while (any(absZ > 2) && iter <= n_iterations){
    ...
    # # Optional: Save progress every s steps
    # if ((iter-burn)%%s==0 && iter > burn) {
    #   save(iter, theta_star_storage, cluster_assignments_storage, num_clusters_storage, absZ_storage, file = paste0(name, "/results_iter_", iter, ".rdata"))
    # }
  ...
  return(list(
    theta_star = theta_star_storage, 
    clusters = cluster_assignments_storage,
    num_clusters = num_clusters_storage, 
    Z = absZ_storage
...
NonParamresult2<-function(name,rawdata)
{
    cputs = maxt = NULL
    NonParamFiles = Sys.glob(paste(name,"/IMMclusters*.Rdata",sep="")) 
    
    finalCs = matrix(NA,nrow=length(NonParamFiles),ncol=ncol(rawdata))
    i = 1
    for (filename in NonParamFiles)
    {
      load(filename)
      last_c = results$clusters[dim(results$clusters)[1],]
      finalCs[i,]=CtoC(renameC(last_c), g0=G0, originalData=rawdata)
      cputs[i]=cput
      i = i + 1
    }
    maxt=max(cputs)
